...
```{r variables ambiente}

# Quiero que esto corra solo en jupyterhub
if (dir.exists("/home/jovyan/work/R/libs")) {
  .libPaths(c("/home/jovyan/work/R/libs", .libPaths()))   
}
...
Sys.setenv(
  "AWS_ACCESS_KEY_ID" = Sys.getenv("ACCESS_KEY"),
...
Leer datos de la EPF

```{r cargar datos desde Minio}
bucket_name <- "desarrollo"
object <- "ooee/clases_ia/datos/data/base-gastos-ix-epf-(formato-csv).csv"

#install.packages("rsample", lib="/home/jovyan/work/R/libs")

# Se lee en formato binario 
raw_obj  <- aws.s3::get_object(object = object, 
                               bucket = bucket_name,
                               use_https = TRUE, 
                               region=""  )

gastos <- readr::read_csv2(I(rawToChar(raw_obj))) 

object <- "ooee/clases_ia/datos/data/base-personas-ix-epf-(formato-csv).csv"
...
personas <- readr::read_csv2(I(rawToChar(raw_obj))) 
...
#install.packages("rsample", lib="/home/jovyan/work/R/libs") por si no la tienen
...
```{r xgboost}
#install.packages("xgboost", lib="/home/jovyan/work/R/libs") por si no lo tienen instalado
library(xgboost)
...
El modelo de machine learning preparado para este ejemplo no tiene el desempeño esperado para ser de utilidad. Realiza las siguientes tareas:

- Mejora el modelo con la información disponible de la EPF. Recuerda que solo debes usar variables que luego tendrás disponible "en producción".
- Haz predicciones en datos de la ESI o CASEN y evalúa el resultado 
...
::: title-area
<h1>Conceptos básicos de machine learning</h1>

<h2>Unidad de Gobierno de Datos - Área de Ciencia de datos</h2>
